...
try {
    ...
    $query = "SELECT * FROM art_piece, art_source, artwork_recieved, department WHERE art_piece.accession_id = artwork_recieved.accession_id AND artwork_recieved.donor_id = art_source.donor_id AND art_piece.department_id = department.department_id AND 1=1";
    if ($title) {
    ...
    if ($startDate && $endDate) {
        $query .= " AND century BETWEEN :startDate AND :endDate";
    }
    ...
    <style>
        body {
            ...
            background-color: #f4e1f4; /* Soft pastel purple background */
            margin: 0;
            padding: 20px;
        }
        table {
    ...
    <body>
    <h1>Art Pieces Results</h1><h2>Click on any column header to sort the table by that column. First 200 results shown below.</h2>";
